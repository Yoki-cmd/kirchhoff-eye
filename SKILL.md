...
**多端件位姿证据优先级（禁止像素过拟合）**：`at/rotate/mirror` 是真相源，按
**清晰主连接 pin 所在主干/母线 > 清晰符号轮廓与方向 > 多个低置信度 pin 像素平均** 定位。
若基极/栅极/运放输入等主连接 pin 明确落在水平或竖直主干上，先让该 pin 与主干严格共线，
再由 `templates/anchors.json` 反推 `at`；不得为了迁就照片中模糊的 C/E、D/S 等末端估计，
把整个元件平移出主干。`pins[].at` 是观测冗余，不是通过平均值覆盖位姿的第二真相源。
小于半网格的离网偏移必须有可指出的原图构图证据（真实短桩、重要错位或离网 pin）；仅由
透视、线宽、JPEG/拍照噪声产生的 0.05/0.1 等残差应吸附到主干/网格。
...
- 多端件先看引脚朝向定 (rotate, mirror)，`pins[].at` 填观测像素换算值（W103 会帮你
  核对位姿有没有看错）。
- 多端件定位先选一个**最高置信度主连接 pin**作为对齐基准：主干坐标明确时，要求该 pin
  与主干严格共 x/y；其余低置信度 pin 只用于核对方向和尺度。若反算结果出现微小离网中心、
  多余 0.05/0.1 短桩或 E003，必须回到原图判断它是构图证据还是照片噪声，禁止直接批准。
...
**警告处置门禁（批准前逐条过）**：不能把 `status:warn` 整体视为无害。逐条读取
`validation.json` 与 `layout_report.json`，为每个 W 写明“保留理由 + 原图证据”或修复它。
以下警告默认阻止“无差异/approved”，除非已有明确取证：落在多端件 `components[].at` 的 E003、
为了接主干新增微小 waypoint 的 E003、W103 位姿观测冲突，以及任何影响主干对齐/元件相对位置的 W。
只有不改变当前语义/构图的画布、标签建议等普通 W 才可带警告批准。审读时必须同时查看
`circuit.debug.png` 的红色锚点，确认主连接 pin 未因照片噪声偏离主干。
...
- 状态语义：`valid`=IR 与产物有效；`needs_review`=待逐区审读或待显式批准；
  `needs_human`=明确 blocker（如 W108 unknown）、收敛止损或轮次上限；按上文门禁逐条确认后，
  只有不影响主干对齐/位姿/拓扑的普通 W 才不阻塞批准；`approved`=零差异审读后显式批准。
...
并写明根因。后续 repair 或重建不得覆盖该目录；证据快照用于区分感知决策、skill 规则与后端实现责任。
