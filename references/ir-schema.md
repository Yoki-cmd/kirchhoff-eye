...
生成 `.debug.tex`；渲染该文件得到 `.debug.png` 后由用户读取坐标。

## 2. 顶层结构

`version, meta, style, routing(可选), nodes(可选), components, wires, junctions, crossings, terminals, texts, regions, nets(可选), unknowns`

除 `nets` 外全部**必填**——没有内容也要写空数组 `[]`。这是刻意设计：强迫感知端对
"这张图没有 junction/crossing/unknown" 做出显式断言，而不是漏写。
...
- `label` = 元件符号名（如 `R_1`），`value` = 数值（如 `10\mathrm{k}\Omega`）；两者都是
  **不带 `$` 的 TeX 数学内容**，限 ASCII+TeX 命令。归属元件的值/标签**禁止**放 texts。
...
9. 图上每段文字：元件 label/value（定 side）/ texts 三选一归属？
10. regions 覆盖全部元件？unknowns 该建的建了？
